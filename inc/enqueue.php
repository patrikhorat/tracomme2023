...
if ( ! function_exists( 'tracomme2023_scripts' ) ) {
	/**
	 * Load theme's JavaScript and CSS sources.
	 */
	function tracomme2023_scripts() {
		// Get the theme data.
		...
		wp_enqueue_style( 'tracomme2023-styles', get_template_directory_uri() . $theme_styles, array(), $css_version );

		// Fix that the offcanvas close icon is hidden behind the admin bar.
		if ( 'bootstrap4' !== $bootstrap_version && is_admin_bar_showing() ) {
			tracomme2023_offcanvas_admin_bar_inline_styles();
		}

		wp_enqueue_script( 'jquery' );

		$js_version = $theme_version . '.' . filemtime( get_template_directory() . $theme_scripts ); // @phpstan-ignore-line -- file exists
		wp_enqueue_script( 'tracomme2023-scripts', get_template_directory_uri() . $theme_scripts, array(), $js_version, true );
		if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
			wp_enqueue_script( 'comment-reply' );
		}
	}
} // End of if function_exists( 'tracomme2023_scripts' ).

add_action( 'wp_enqueue_scripts', 'tracomme2023_scripts' );

if ( ! function_exists( 'tracomme2023_offcanvas_admin_bar_inline_styles' ) ) {
	/**
	 ...
	 * @since 1.2.0
	 */
	function tracomme2023_offcanvas_admin_bar_inline_styles() {
		...
		wp_add_inline_style( 'tracomme2023-styles', $css );
	}
}
